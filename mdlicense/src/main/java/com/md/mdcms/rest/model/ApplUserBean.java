...
import com.md.mdcms.rest.base.Function;
import com.md.mdcms.rest.base.IFunction;
import com.md.mdcms.rest.base.MdFunction;
...
public class ApplUserBean extends UserBean implements IConstants, Xml {
	...
	public OutcomeObject processFunction() {
		synchronized (this) {
			...
			// FacesContext facesContext = FacesContext.getCurrentInstance();
			try {
				function = new Function(this, this.requestBackingBean);
	 ...
	 */
	private String getJsDateSeparator() {
	...
	// .generateXmlRequestString(new
	// Operation(sessionLogoffState.getAsGlobalContainer()));
	//
	// XmlResponseObject xmlResponseObject = runMiddleware(jobNumber,
	// sessionLogoffState, xmlRequestString, false);
	//
	// // XmlResponseObject responseObject = iSeries
	// // .runMiddleware(xmlRequestString);
	//
	// Operation responseOperation =
	// ResponseXml.generateOperation(xmlResponseObject.getXmlResponseString());
	// screen =
	// responseOperation.getContainer(CONTAINERTYPEGLOBAL).getField(SCREEN).getValue();
	// } catch (XMLStreamException e) {
	// LOG.fatal(e);
	// } catch (IOException e) {
	// LOG.fatal(e);
