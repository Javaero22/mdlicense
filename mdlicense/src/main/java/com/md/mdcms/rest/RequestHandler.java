...
public class RequestHandler implements IConstants, Xml {

	/** Log instance for this class. */
	protected static final Log LOG = LogFactory.getLog(RequestHandler.class);
	...
	private IBackingBean backingBeanManager;

	public RequestHandler() {
		super();
	}

	public IBackingBean handleRequest(HttpServletRequest request, String jobNumber,
			Class<?> classToCreate) {
		try {
			backingBeanManager = (IBackingBean) classToCreate.newInstance();

			backingBeanManager.setJobNumber(jobNumber);
	...
	public boolean create() {
		...
		if (this.responseOperation != null) {
			...
			if (cont != null) {
				...
				/*
				 * 2 - needs to be after the setRequestResponseCodes because of
				 * the browserendered stuff
				 */
